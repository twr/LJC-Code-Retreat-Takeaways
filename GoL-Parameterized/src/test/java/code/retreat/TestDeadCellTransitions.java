...
public class TestDeadCellTransitions {

	private final int liveNeighbours;

	private final Cell expected;

	@Parameters(name = "{index}: a dead cell with {0} neighbours is {1} after transition")
	...
	public TestDeadCellTransitions(int liveNeighbours, Cell expected) {
	...
	public void assertExpectedResult() {
		// when
		Cell result = DEAD.mutate(liveNeighbours);
